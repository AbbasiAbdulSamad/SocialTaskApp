import 'package:app/ui/layout_widgets/website_layout.dart';
...
class Web_visitors extends StatelessWidget {
  ...
  final ValueNotifier<String?> selectedCountry = ValueNotifier<String?>(null);
  final ValueNotifier<int?> selectedTime = ValueNotifier<int?>(10);
  final ValueNotifier<int> totalNotifier = ValueNotifier<int>(0);
  final ValueNotifier<int> discountNotifier = ValueNotifier<int>(0);

  //  Quantity calucation function
  void updateTotal(BuildContext context) {
    ...
    final subscriber = subscriberValue * 3;
    int watchtime = 0;
    if(selectedValue==10){watchtime = 1 * subscriberValue;
    }else if(selectedValue==20){watchtime = 2 * subscriberValue;
    }else if(selectedValue==30){watchtime = 3 * subscriberValue;
    }else if(selectedValue==45){watchtime = 4 * subscriberValue;
    }else if(selectedValue==60){watchtime = 6 * subscriberValue;
    }else if(selectedValue==90){watchtime = 9 * subscriberValue;
    }else if(selectedValue==120){watchtime = 12 * subscriberValue;
    }else if(selectedValue==150){watchtime = 15 * subscriberValue;
    }else if(selectedValue==180){watchtime = 18 * subscriberValue;
    }else if(selectedValue==210){watchtime = 21 * subscriberValue;
    }else if(selectedValue==240){watchtime = 24 * subscriberValue;
    }else if(selectedValue==270){watchtime = 27 * subscriberValue;
    }else if(selectedValue==300){watchtime = 30 * subscriberValue;
    }else{watchtime = 0 * subscriberValue;}
  ...
  void buttonAction(BuildContext context) async {
    if (_formKey.currentState!.validate()) {
      ...
      await campaignProvider.createCampaign(
        ...
        title: webTitle!,
        videoUrl: webLink!,
        watchTime: selectedTime.value ?? 10,
        quantity: int.tryParse(quantitySubscribers.text) ?? 0,
        selectedOption: "Visitors",
        campaignImg: webIcon!,
        social: "Website",
              ...
              child: Column(
                ...
                children: [
                  ...
                  WebsiteLayout(
                    ...
                    ytService: 'Visitors',
                    ytServiceIcon: CupertinoIcons.globe
                  ),
                ],
              ),
            );
